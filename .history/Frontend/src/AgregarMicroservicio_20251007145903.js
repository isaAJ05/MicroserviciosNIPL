...
if __name__ == "__main__":
  ...
  "consulta_roble": `# Microservicio Consulta Tabla Roble (usa token recibido por header)
def main(data=None):
    """
    Consulta una tabla en Roble usando el token recibido por header y parámetros recibidos en el JSON o query.
    """
    import requests
    try:
        ...
        for k, v in (data or {}).items():
            if k not in ["roble_token", "tableName"]:
                params[k] = v

        # Consulta la tabla en Roble
        res = requests.get(
            f"https://roble-api.openlab.uninorte.edu.co/database/TU_TOKEN_CONTRACT/read",
