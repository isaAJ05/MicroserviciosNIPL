...
function PanelPrincipal() {
  ...
  const [editMicroservice, setEditMicroservice] = useState(null);
  ...
  const handleSaveEdit = async (e) => {
    e.preventDefault();
    const res = await fetch(`http://127.0.0.1:5000/microservices/${editMicroservice.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(editMicroservice)
    });
    if (res.ok) {
      setEditMicroservice(null);
      refreshMicroservices();
    }
  };
// Función para abrir el modal de edición
  const handleEdit = (microservice) => {
    setEditMicroservice(microservice);
  useEffect(() => {
              ...
              {microservices.map(microservice => (
                <tr key={microservice.id}>
                  ...
                  <td>
                    ...
                    <button className="action-btn" title="Editar" onClick={() => handleEdit(microservice)}>✏️</button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="modal-bg">
          <div className="modal">
            ...
            <form onSubmit={handleSaveEdit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <input
                type="text"
                value={editMicroservice.name}
                onChange={e => setEditMicroservice({ ...editMicroservice, name: e.target.value })}
                required
              />
              <input
                type="text"
                value={editMicroservice.processing_type}
                onChange={e => setEditMicroservice({ ...editMicroservice, processing_type: e.target.value })}
                required
              />
              <textarea
                value={editMicroservice.code}
                onChange={e => setEditMicroservice({ ...editMicroservice, code: e.target.value })}
                rows={5}
                required
              />
              <div style={{ display: 'flex', gap: 10 }}>
                <button type="submit" className="action-btn">Guardar</button>
                <button type="button" className="action-btn" style={{ background: '#23263a' }} onClick={() => setEditMicroservice(null)}>Cancelar</button>
              </div>
            </form>
          </div>
          </div>
