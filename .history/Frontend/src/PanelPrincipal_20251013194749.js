...
function PanelPrincipal() {
  ...
  if (showInfoPage) {
    return (
      <div>
        <button onClick={() => setShowInfoPage(false)} style={{ margin: '10px', padding: '8px 16px', cursor: 'pointer' }}>
          Volver
        </button>
        <Informacion />
      </div>
          ...
          >
            ...
            {isHistoryOpen && (
              <ul className="sidebar-list">
                <li style={{ cursor: 'pointer' }} onClick={() => setShowInfoPage(true)}>
