...
function PanelPrincipal() {
  ...
  const [renewTokenLoading, setRenewTokenLoading] = useState(false);
  const [renewTokenError, setRenewTokenError] = useState("");
        ...
        <div className="modal-bg" style={{
          ...
          <div className="modal" style={{ width: 400, maxWidth: '90vw', minWidth: 280, padding: 28, textAlign: 'center' }}>
            ...
            <form
              onSubmit={async e => {
                e.preventDefault();
                setRenewTokenLoading(true);
                setRenewTokenError("");
                try {
                  const username = user ? (user.username || user.name || user.email) : '';
                  const projectId = localStorage.getItem('tokenContract') || '';
                  // Ajusta la URL y payload según tu backend
                  const res = await fetch('http://127.0.0.1:5000/renew_token', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                      username,
                      password: renewTokenPassword,
                      project_id: projectId
                    })
                  });
                  if (!res.ok) {
                    const errData = await res.json().catch(() => ({}));
                    throw new Error(errData.message || 'Error al renovar el token');
                  }
                  ...
                  setRenewTokenError(err.message);
                } finally {
                  setRenewTokenLoading(false);
                }
              }}
            ...
            >
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 6 }}>
                ...
                <input
                  ...
                  disabled
                  style={{ width: '100%', background: '#eee', color: '#888', borderRadius: 6, padding: 8, border: '1px solid #ccc' }}
                />
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 6 }}>
                ...
                <input
                  type="text"
                  value={localStorage.getItem('tokenContract') || ''}
                  disabled
                  style={{ width: '100%', background: '#eee', color: '#888', borderRadius: 6, padding: 8, border: '1px solid #ccc' }}
                />
              </div>
              ...
              {renewTokenError && (
                <div style={{ color: '#b91c1c', fontWeight: 600, marginTop: 4 }}>{renewTokenError}</div>
              )}
              <div style={{ display: 'flex', gap: 10, marginTop: 8, justifyContent: 'center' }}>
                <button type="submit" className="action-btn" disabled={renewTokenLoading}>
                  {renewTokenLoading ? 'Renovando...' : 'Renovar'}
                </button>
                <button type="button" className="action-btn" style={{ background: '#23263a' }} onClick={() => { setShowRenewTokenModal(false); setRenewTokenPassword(""); setRenewTokenError(""); }}>
                  Cancelar
                </button>
              </div>
            </form>
