    ...
    <ul className="sidebar-list">
      ...
      {showDeleteModal && (
        <div className="modal-bg" style={{ 
          ...
          WebkitBackdropFilter: 'blur(2.5px                            let url = `http://localhost:${microservice.port}/${microservice.endpoint}`;
                            if (microservice.processing_type === "Suma") {
                              url += `?a=5&b=3`;
                            } padding: 28, textAlign: 'centif (microservice.processing_type === "Consulta Roble") {
                              url += `?tableName=inventario`;
                            }onfirmar Eliminación
            </h3>
            <div style={{ fontSize: 16, marginBosetShowEditEndpointUrlModal ¿Estás seguro de que deseas eliminar este microservicio?
            </div>
...
ff9696const token = (localStorage.getItem('accessToken') || '').trim();
const tokenContract = (localStorage.getItem('tokenContract') || '').trim();
const res = await fetch(customUrl, {
  method: 'GET',
  headers: {
    'Authorization': `Bearer ${token}`,
    'Token-Contract': tokenContract // <--- así lo envías por header
  }
});                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          ,
                                                  }