...
function PanelPrincipal() {
  ...
  return (
    <>
      ...
      {/* Contenido principal */}
      <div className={`app-container${loginFade || !isLoggedIn ? '' : ' main-fade-in'}${isHistoryOpen ? ' history-open' : ''}${lightTheme ? ' light-theme' : ''}`}
            style={{
...
@keyframes fadeOutMsg {
  ...
  100% { opacity: 0; transform: translateY(-18px) scale(0.95); filter: blur(6px); }
}

.fade-out {
  animation: fadeOutMsg 0.7s cubic-bezier(0.4,0,0.2,1) forwards;
}
.main-fade-in {
  animation: fadeInMsg 0.7s cubic-bezier(0.4,0,0.2,1) forwards;
}
.show-register {
  animation: fadeInMsg 0.5s cubic-bezier(0.4,0,0.2,1) forwards;
}
.hide-register {
  animation: fadeOutMsg 0.5s cubic-bezier(0.4,0,0.2,1) forwards;
}`;
