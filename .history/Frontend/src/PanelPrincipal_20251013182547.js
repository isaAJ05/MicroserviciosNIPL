...
function PanelPrincipal() {
  ...
  // Leer variable de entorno para URL del backend
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/is_docker_active`)
      .then(res => res.json())
      .then(data => {
  ...
  const handleDelete = async () => {
    ...
    await fetch(`${API_URL}/microservices/${microserviceToDelete}`, { method: 'DELETE' });
                      ...
                      <td style={{ display: 'flex', gap: 6 }}>
                        <button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              const res = await fetch(`${API_URL}/microservices/${microservice.id}/mainpy`);
