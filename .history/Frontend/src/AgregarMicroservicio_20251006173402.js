...
function PythonEditor({ code, setCode, lightTheme }) {
  ...
  // Inicializa el editor (solo al montar o si cambia tema)
  useEffect(() => {
    if (editorRef.current && !viewRef.current) {
      const extensions = [
        ...
        EditorView.updateListener.of((update) => {
          // Cuando el usuario escribe, actualizamos el estado externo con debounce
          if (update.docChanged && !updatingFromOutside.current) {
            const newText = update.state.doc.toString();
            if (debounceRef.current) clearTimeout(debounceRef.current);
            debounceRef.current = setTimeout(() => {
              setCode(newText);
            }, 300); // ajustar ms según prefieras
          }
        })
      ];
  ...
  }, [lightTheme, setCode]); // no dependemos de `code` aquí para evitar reinicios constantes

  // Aplica cambios externos (ej. seleccionar ejemplo) SOLO si el editor NO tiene foco
  useEffect(() => {
    if (!viewRef.current) return;
    const currentCode = viewRef.current.state.doc.toString();
    if (currentCode === code) return;
    // evita sobrescribir si el usuario está editando (tiene foco)
