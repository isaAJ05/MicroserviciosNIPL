...
//MICROSERVICIOS
const [microservices, setMicroservices] = useState([]); // Lista de microservicios
const [newMicroservice, setNewMicroservice] = useState({
  name: "",
  endpoint: "",
  processing_type: "",
}); // Datos del formulario
// Obtener la lista de microservicios
const fetchMicroservices = async () => {
  try {
    const response = await fetch("http://127.0.0.1:5000/microservices");
    const data = await response.json();
    setMicroservices(data); // Actualiza el estado con la lista de microservicios
  } catch (error) {
    console.error("Error al obtener los microservicios:", error);
  }
};
...
const createMicroservice = async () => {
  try {
    ...
    if (response.ok) {
      fetchMicroservices(); // Actualiza la lista después de crear
      setNewMicroservice({ name: "", endpoint: "", processing_type: "" }); // Limpia el formulario
    } else {
      const errorData = await response.json();
      console.error("Error al crear el microservicio:", errorData);
    }
  } catch (error) {
...
const deleteMicroservice = async (id) => {
  try {
    const response = await fetch(`http://127.0.0.1:5000/microservices/${id}`, {
      method: "DELETE",
    });
    if (response.ok) {
      fetchMicroservices(); // Actualiza la lista después de eliminar
    } else {
      const errorData = await response.json();
      console.error("Error al eliminar el microservicio:", errorData);
    }
  } catch (error) {
    console.error("Error al eliminar el microservicio:", error);
  }
};
