        ...
        <tbody>
          {microservices.map(microservice => (
            <tr key={microservice.id}>
              ...
              <td>
                {/* Asumiendo que el puerto es 8000 + id */}
                <a
                  href={`http://localhost:${8000 + microservice.id}/process`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="action-btn"
                  style={{ textDecoration: 'underline', color: '#1a73e8', fontWeight: 600 }}
                >
                  Ver Endpoint
                </a>
              <button
                className="action-btn"
                title="Ver Endpoint"
                onClick={() => window.open(`http://localhost:${8000 + microservice.id}/process`, '_blank')}
                style={{ marginRight: 8, background: '#1a73e8', color: '#fff', borderRadius: 4, border: 'none', padding: '6px 10px', fontWeight: 600, cursor: 'pointer' }}
