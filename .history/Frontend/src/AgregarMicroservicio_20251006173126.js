...
function PythonEditor({ code, setCode, lightTheme }) {
  ...
  // Solo inicializa el editor una vez
  useEffect(() => {
      ...
      const extensions = [
        ...
        EditorView.updateListener.of((update) => {
          if (update.docChanged && !updatingFromOutside.current) {
            // Solo actualiza el estado externo cuando el cambio viene de fuera (ejemplo seleccionado)
            // No actualices el estado externo en cada pulsación
  ...
  // Solo depende de lightTheme y code externo
  }, [code, lightTheme]);
