...
import os
from flask import Flask, request, jsonify
from models import create_microservice, list_microservices, update_microservice, delete_microservice_by_id
import subprocess
import uuid
import tempfile
...
def get_microservices():
    internal_list = list_microservices()
    result = subprocess.run(
        ["docker", "ps", "--format", "{{.Image}}|{{.Names}}|{{.Status}}|{{.Ports}}"],
        capture_output=True,
        text=True,
        timeout=5
    )
    ...
    for line in result.stdout.strip().split('\n'):
        if line:
            image, name, status, ports = line.split('|')
            # Extrae el puerto externo de la cadena de puertos
            # Ejemplo: '0.0.0.0:8002->8000/tcp' => 8002
            port = None
            ...
            docker_list.append({
                ...
                "status": status,
                "ports": ports,
                "port": port  # <-- puerto real de Docker
            })

    microservices = []
    for ms in internal_list:
        docker_info = next((d for d in docker_list if d["image_name"] == ms["image_name"]), None)
        ms_copy = ms.copy()
        if docker_info:
            ms_copy.update({
                ...
                "port": docker_info["port"]  # <-- usa el puerto real
            })
        else:
            ms_copy.update({
                "container_name": None,
...
def add_microservice():
    ...
    # Calcula el id que tendrá el microservicio
    from models import microservices
    ms_id = len(microservices) + 1
    port = 8000 + ms_id

    # Guarda el microservicio en memoria
    microservice = create_microservice(name, endpoint, processing_type, code, port)
    ...
    flask_code = generar_codigo_flask(nombre_funcion, code, endpoint)
...
"""
    ...
    # Obtén el puerto real desde Docker
    inspect_cmd = ["docker", "inspect", "--format", "{{(index (index .NetworkSettings.Ports \"8000/tcp\") 0).HostPort}}", image_name]
    result = subprocess.run(inspect_cmd, capture_output=True, text=True)
    real_port = result.stdout.strip()
    # Actualiza el microservicio en la lista interna
    for ms in microservices:
        if ms["image_name"] == image_name:
            ms["port"] = real_port
            break
    try:
        subprocess.run(run_cmd, check=True)
...
def delete_microservice(ms_id):
    print("Lista interna:", list_microservices())
    print("ID recibido:", ms_id)
    # Busca el microservicio en la lista interna
    ms = next((m for m in list_microservices() if m["id"] == ms_id), None)
    if not ms:
        return jsonify({"error": "Microservicio no encontrado"}), 404

    image_name = ms["image_name"]  # Usa el nombre real del contenedor
    ...
    # Elimina el microservicio de la lista interna
    delete_microservice_by_id(ms_id)
