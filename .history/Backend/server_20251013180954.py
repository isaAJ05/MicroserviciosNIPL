...
def is_docker_running():
    """Helper que devuelve True/False según si Docker responde.

    Usar esta función cuando necesitemos una comprobación booleana
    desde otras funciones del módulo. La ruta HTTP `/is_docker_active`
    puede reutilizarla para devolver JSON al cliente.
    """
    try:
...
def is_docker_active():
    # Ruta que devuelve JSON; delega la lógica al helper booleano
    return jsonify({"active": is_docker_running()})
